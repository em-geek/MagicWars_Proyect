...
public class HelloAnimation extends SimpleApplication {
  ...
  public void simpleInitApp() {
    ...
    Node player = (Node) assetManager.loadModel("Models/a-cartoony-female-character/a-cartoony-female-character.j3o");
    player.setLocalScale(0.5f);
    rootNode.attachChild(player);

    /* Use the model's AnimComposer to play its "stand" animation clip. */
    System.out.println("Nombres de los hijos de player: " + player.getChildren());
    control = player.getChild("Armature").getControl(AnimComposer.class);
    
    
    //
    System.out.println("Nombre del AnimComposer: " + control.getLayerNames());

    // Imprimir nombres de todos los clips de animación disponibles
    for (String clipName : control.getAnimClipsNames()) {
        System.out.println("Clip de animación disponible: " + clipName);
    }

    // Imprimir nombres de todas las capas de animación
    for (String layerName : control.getLayerNames()) {
        System.out.println("Capa de animación: " + layerName);
    }

    // Imprimir nombres de todas las animaciones en todas las capas
    

// Imprimir otras propiedades del AnimComposer, si es necesario
// Por ejemplo, duración de los clips, velocidades de reproducción, etc.
